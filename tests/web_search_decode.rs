use lingxi_agent_api::protocol::{ContentBlock, StopReason, StreamEvent};
...
fn searches(events: &[StreamEvent]) -> Vec<&lingxi_agent_api::protocol::WebSearchResult> {
    events
        .iter()
...
fn responses_preserves_annotations_search_actions_and_local_tools() {
    ...
    let result = OpenAiResponsesCodec.decode_response(&response(json!({"output":[call,{"type":"message","content":[{"type":"output_text","text":"News","annotations":[a]}]},{"type":"function_call","call_id":"local","name":"save","arguments":"{}"}]}))).unwrap();
...
fn responses_stream_deduplicates_terminal_annotations_and_calls() {
    ...
    let events: Vec<_> = frames
        .iter()
        .flat_map(|f| decoder.decode_frame(f.to_string().as_bytes()).unwrap())
        .collect();
...
fn responses_terminal_only_annotations_are_not_lost() {
    let mut decoder = OpenAiResponsesCodec.stream_decoder();
    let events = decoder.decode_frame(json!({"type":"response.completed","response":{"output":[{"type":"message","content":[{"annotations":[annotation()]}]}]}}).to_string().as_bytes()).unwrap();
...
fn responses_top_level_server_usage_survives_complete_and_stream() {
    ...
    let decoded = OpenAiResponsesCodec
        .decode_response(&response(body.clone()))
        .unwrap();
    assert_eq!(
...
fn chat_and_openrouter_nested_annotations_survive_streaming() {
    ...
    let result = OpenAiChatCodec.decode_response(&response(json!({"choices":[{"message":{"content":"News","annotations":[a]},"finish_reason":"stop"}]}))).unwrap();
    ...
    assert_eq!(
        searches(&decoder.decode_frame(frame.as_bytes()).unwrap())[0]
            .citations
            .len(),
        1
    );
    assert!(searches(&decoder.decode_frame(frame.as_bytes()).unwrap()).is_empty());
}
...
fn anthropic_search_errors_and_encrypted_attribution_are_preserved() {
    ...
    let result = AnthropicMessagesCodec
        .decode_response(&response(
            json!({"content":[error,cited],"stop_reason":"end_turn"}),
        ))
        .unwrap();
    let search = result.web_search.unwrap();
    ...
    let error_only = AnthropicMessagesCodec
        .decode_response(&response(json!({"content":[error]})))
        .unwrap()
        .web_search
        .unwrap();
...
fn ordinary_responses_do_not_claim_web_search() {
    ...
    assert!(OpenAiResponsesCodec
        .decode_response(&response(json!({"output":[]})))
        .unwrap()
        .web_search
        .is_none());
    assert!(AnthropicMessagesCodec
        ...
        .is_none());
    assert!(GeminiCodec
        .decode_response(&response(json!({"candidates":[{}]})))
        .unwrap()
        .web_search
        .is_none());
}
...
fn anthropic_pause_turn_and_citations_replay_exact_native_blocks() {
    use lingxi_agent_api::protocol::{
        CompletionRequest, FailoverTriggers, ModelCapabilities, ProviderId, ProviderProfile,
        ToolChoice,
    };
    ...
    let decoded = AnthropicMessagesCodec
        .decode_response(&response(
            json!({"content":blocks,"stop_reason":"pause_turn"}),
        ))
        .unwrap();
    ...
    let encoded = AnthropicMessagesCodec
        .encode_request(&req, &profile, &route, &RequestOptions::default())
        .unwrap();
...
fn native_search_counters_survive_full_and_usage_only_stream_frames() {
    ...
    let decoded = AnthropicMessagesCodec
        .decode_response(&response(
            json!({"content":[],"usage":{"server_tool_use":counters}}),
        ))
        .unwrap();
    assert_eq!(
