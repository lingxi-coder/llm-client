//! `ProviderProfile`: the JSON shape of one provider entry in settings. Same
//! shape as the previous project's `llm-client/src/config.rs`; the one
...
pub enum CredentialConfig {
    ...
    /// Looked up in the platform's `CredentialStore` under `key`.
    HostManaged {
        key: String,
    },
    #[default]
...
/// Ported from the previous project's `platform_api::ModelBillingMode`: it is
/// plain data, so it belongs in protocol, not behind a platform trait.
#[derive(
...
/// routing. Ported from `platform_api::ModelMetadata`.
...
/// Published prices for one model, in USD per million tokens.
///
...
impl TokenPricing {
    /// The rates in force for `submission` at `unix_seconds`, given the
    /// provider's schedule.
    ///
    /// A batch submission starts from the published batch rates when the model
    /// carries them and from the listed rates when it does not: an unpriced
    /// batch is not a free batch, and quoting the dearer interactive rate is
    /// the safe direction to be wrong in. A caller that needs to know which of
    /// the two it got asks `pricing.batch.is_some()`.
    ///
    /// The schedule then applies on top. No vendor in the shipped catalog
    /// declares both a batch table and a peak schedule, so that composition has
    /// never been checked against a real bill — verify it before trusting it if
    /// one ever declares both.
    ///
    /// No schedule, or a time inside peak, leaves the selected rates unchanged.
    /// The result carries no `batch` of its own: it is already resolved, and a
    /// resolved rate has no sub-rate.
    #[must_use]
    pub fn at(
        &self,
        ...
        let base = match (submission, &self.batch) {
            (Submission::Batch, Some(b)) => TokenPricing {
                input_per_million: b.input_per_million,
                output_per_million: b.output_per_million,
                cache_read_per_million: b.cache_read_per_million,
                cache_write_per_million: b.cache_write_per_million,
                reasoning_per_million: b.reasoning_per_million,
        ...
        if schedule.is_peak(unix_seconds) {
            return base;
        }
        let scale = |v: Option<f64>| v.map(|v| v * schedule.off_peak_multiplier);
        TokenPricing {
            input_per_million: scale(base.input_per_million),
            output_per_million: scale(base.output_per_million),
            cache_read_per_million: scale(base.cache_read_per_million),
            cache_write_per_million: scale(base.cache_write_per_million),
            reasoning_per_million: scale(base.reasoning_per_million),
            source: base.source,
            batch: None,
        }
    }
...
pub struct ProviderProfile {
    ...
    /// and appends the rest. This crate does not name providers (gate 47), and
    /// the codec crate is where that knowledge belongs anyway.
    pub base_url: String,
    ...
    /// Same-profile model to route image input to when the selected model
    /// cannot accept it.
    #[serde(default, rename = "visionDelegate")]
...
impl ProviderProfile {
    ...
    /// total and stable across runs — which `--dump-registry` needs (gate 42).
    #[must_use]
...
mod pricing_tests {
    ...
    #[test]
    fn the_discount_reaches_every_bucket() {
        let listed = TokenPricing {
            input_per_million: Some(0.3),
            output_per_million: Some(1.2),
            cache_read_per_million: Some(0.006),
            cache_write_per_million: Some(0.4),
            reasoning_per_million: Some(1.2),
            source: Some("test".to_owned()),
            batch: None,
        };
        let off = listed.at(Submission::Interactive, Some(&schedule()), at(5, 2, 0));
        assert_eq!(off.input_per_million, Some(0.15));
        assert_eq!(off.output_per_million, Some(0.6));
        assert_eq!(off.cache_read_per_million, Some(0.003));
        assert_eq!(off.cache_write_per_million, Some(0.2));
        assert_eq!(off.reasoning_per_million, Some(0.6));
        assert_eq!(
            off.source, listed.source,
            "provenance survives the discount"
        );
    }

    #[test]
    fn an_absent_price_stays_absent_rather_than_becoming_zero() {
        let unpriced = TokenPricing::default();
        let off = unpriced.at(Submission::Interactive, Some(&schedule()), at(5, 2, 0));
        assert_eq!(
            off.input_per_million, None,
            "unpriced is not free, and half of unknown is still unknown"
        );
    }

    fn priced_both_ways() -> TokenPricing {
        TokenPricing {
    ...
    #[test]
    fn a_batch_submission_is_billed_at_the_published_batch_rates() {
        let p = priced_both_ways();
        let b = p.at(Submission::Batch, None, at(0, 2, 0));
        assert_eq!(b.input_per_million, Some(2.5));
        assert_eq!(b.output_per_million, Some(12.5));
        assert_eq!(b.cache_read_per_million, Some(0.25));
        assert_eq!(b.cache_write_per_million, Some(3.125));
        assert_eq!(
            p.at(Submission::Interactive, None, at(0, 2, 0))
                .input_per_million,
            Some(5.0),
            "the same model interactively is still the listed rate"
        );
    }

    #[test]
    fn a_batch_submission_with_no_batch_rates_quotes_the_listed_ones() {
        // Not free, and not silently halved: an unpriced batch is unknown, and
        // the dearer interactive rate is the safe direction to be wrong in.
        // `pricing.batch.is_some()` is how a caller tells the two apart.
        let mut p = priced_both_ways();
        p.batch = None;
        assert_eq!(
            p.at(Submission::Batch, None, at(0, 2, 0)).input_per_million,
            Some(5.0)
        );
    }

    #[test]
    fn a_resolved_rate_carries_no_sub_rate() {
        // The result is what you pay, not a table to resolve again.
        for s in [Submission::Interactive, Submission::Batch] {
            assert_eq!(priced_both_ways().at(s, None, at(0, 2, 0)).batch, None);
        }
    }

    #[test]
    fn the_schedule_applies_on_top_of_the_batch_rates() {
        // No shipped vendor declares both, so this pins the composition rather
        // than reporting a verified bill: batch selects the base rates, then
        // the schedule scales them.
        let off = priced_both_ways().at(Submission::Batch, Some(&schedule()), at(5, 2, 0));
        assert_eq!(off.input_per_million, Some(1.25));
        let peak = priced_both_ways().at(Submission::Batch, Some(&schedule()), at(0, 2, 0));
        assert_eq!(peak.input_per_million, Some(2.5));
    }
}
