//! A value whose `Debug` is always `<redacted>`. Credentials never leave a
//! `CredentialStore` in any other wrapper. Deserializable (a settings file may
//! carry a static credential), never serializable.
