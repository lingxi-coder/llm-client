...
pub(crate) fn report(
    raw: Option<&Value>,
    shape: &ReportShape,
    decode: fn(&Value) -> crate::protocol::Usage,
    final_report: bool,
) -> crate::protocol::UsageReport {
    use crate::protocol::{UsageReport, UsageState};
    let Some(raw) = raw.filter(|raw| !raw.is_null()) else {
        return UsageReport::default();
    };
    let malformed = !raw.is_object()
        ...
        || shape
            .subsets
            .iter()
            .any(|(path, _)| raw.pointer(path).is_some_and(|v| v.as_u64().is_none()))
        || shape
            .independent_counters
            .iter()
            .any(|key| raw.get(key).is_some_and(|v| v.as_u64().is_none()));
    let malformed = malformed
        ...
        || shape
            .thoughts
            .is_some_and(|key| raw.get(key).is_some_and(|v| v.as_u64().is_none()))
        || shape.cache_creation.is_some_and(|(group, total, parts)| {
            raw.get(group).is_some_and(|value| {
                if value.is_null() {
                    return counter(raw, total).is_some_and(|value| value != 0);
                }
                !value.is_object()
                    || parts
                        .iter()
                        .any(|key| value.get(key).is_some_and(|v| v.as_u64().is_none()))
            })
        });
    let complete = is_complete(raw, shape);
    let invalid = malformed
        || (counter(raw, shape.input).is_some()
            && counter(raw, shape.output).is_some()
            && !complete);
    UsageReport::measured(
        decode(raw),
        if invalid {
            UsageState::Invalid
        } else if complete && final_report {
            UsageState::Complete
        } else {
            UsageState::Partial
        },
