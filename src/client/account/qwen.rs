...
use super::{
    ...
    AccountFailure, AccountIdentity, AccountMetric, AccountQuery, AccountQuotaWindow, AccountScope,
    AccountScopeKind, AccountSnapshot, AccountUsageSource,
};
...
impl AccountUsageSource for Qwen {
    async fn fetch(
        ...
        snapshot.quota_windows = match (&query.credential, workspace) {
...
async fn fetch_quotas(
    http: &dyn Transport,
    endpoint: &str,
    key: &lingxi_agent_api::protocol::Secret<String>,
...
async fn signed_get_json(
    ...
    let parsed = reqwest::Url::parse(url).map_err(|_| AccountFailure::InvalidResponse)?;
